...
> 这里内容节选 [这里](https://github.com/ant-design/intl-example/blob/master/docs/understanding-antd-i18n.md)
...
<script>
  ...
  if (!window.Intl) {
    document.writeln('<script src="http://design.yonyoucloud.com/static/intl/1.0.1/Intl.js">' + '<' + '/script>');
    document.writeln('<script src="http://design.yonyoucloud.com/static/intl/1.0.1/locale-data/en.js">' + '<' + '/script>');
  }
</script>
```
...
```javascript
// 暂未实现
import datePickerLocale from 'bee-datepicker/locale/en_US';
