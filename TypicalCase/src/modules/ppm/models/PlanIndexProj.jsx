...
export default {
  ...
  effects: {
    ...
    async load_proj(ids, getState) {
      let param = {
        page:
          getState().PlanIndexProj.number == 0
            ? 0
            : getState().PlanIndexProj.number - 1,
      ...
      if (ids[0] == undefined) {
        ...
        param.searchParams.searchMap.index_code =
          searchParam.code == "" ? null : searchParam.code;
        param.searchParams.searchMap.index_name =
          searchParam.name == "" ? null : searchParam.name;
      } else {
    ...
    async onChangePageIndex(page, getState) {
      ...
      actions.PlanIndexProj.load_proj([
        getState().PlanIndexProj.classfication_id
      ]);
    },
    ...
    showAddModal(data, getState) {
      if (
        getState().PlanIndexProj.modalstate == "add" &&
        (getState().PlanIndexProj.classfication_id == undefined ||
          getState().PlanIndexProj.classfication_id == "")
      ) {
        Error("未选中分类，无法新增项目！");
      } else {
        ...
        if (PlanIndexProj.values == undefined) {
          PlanIndexProj = {
            values: {
              ...
              isauto: {},
              classfication_id: {
                value: getState().PlanIndexProj.classfication_id
              },
      ...
      const loop = data =>
        data.map(item => {
          if (item.values.children) {
            return (
              <TreeNode
                title={item.values.classfication_name.value}
                key={item.values.id.value}
          ...
          return (
            <TreeNode
              title={item.values.classfication_name.value}
              key={item.values.id.value}
              isLeaf={item.values.CONNECT_BY_ISLEAF.value == 1}
            />
          );
        });
